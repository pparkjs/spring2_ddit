...
<body>
	...
	<p>
		...
		<font color="red">서버쪽 컨트롤러 메소드에서 @PathVariable를 사용하지 않는 경우 파라미터로 해당 값을 얻을 수 없다. (null)</font>
	</p>
	...
	<p>5) HTML Form 필드가 여러 개일 경우에 컨트룰로 매개변수의 위치는 상관이 있는가?</p>
	...
	<p>2) 쿼리 파라미터(dateOfBirth=2018-09-08)로 전달받은 값이 날짜 문자열형식으로 설정 시, Date 타입으로 받는 가?</p>
	<a href="/registerByGet01?userId=hongkd&dateOfBirth=2018-09-08">BUTTON5</a>
	
	<p>3) 쿼리 파라미터(dateOfBirth=20180908)로 전달받은 값이 날짜 문자열 형식으로 설정 시, Date 타입으로 받는 가?</p>
	<a href="/registerByGet01?userId=hongkd&dateOfBirth=20180908">BUTTON6</a>
	
	<p>
		4) 쿼리 파라미터(dateOfBirth=2018/09/08)로 전달받은 값이 날짜 문자열 형식으로 설정 시, Date 타입으로 받는 가?<br>
		<font color="red">SUCCESS</font>
	</p>
	<a href="/registerByGet01?userId=hongkd&dateOfBirth=2018/09/08">BUTTON7</a>
	
	<p>5) Member 매개변수와 쿼리 파라미터(dateOfBirth=20180908)로 전달받은 값이 날짜 문자열 형식으로 설정 시, Date 타입으로 받는 가?</p>
	<a href="/registerByGet02?userId=hongkd&dateOfBirth=20180908">BUTTON8</a>
	
	<p>6) Member 매개변수와 폼 방식 요청 전달받은 값이 날짜 문자열 형식으로 설정 시, Date 타입으로 받는 가?</p>
