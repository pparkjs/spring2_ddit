...
public class JSPHomeController {
	 ...
	 * 			prefix		|	태그 라이브러리를 호출할 떄 사용할 접두어를 자정한다.
	 ...
	 * 
	 * 2. 스크립틀릿
	 * - 스크립틀릿(Scriptlet)은 JSP페이지에서 자바 코드를 실행할 때 사용하는코드의 블록이다.
	 ...
	 * 그러고, 커스텀 태그를 모아 놓은 것을 커스텀 태그 라이브러리라고 한다.
	 * 
	 * # 대표적인 태그 라이브러리
	 ...
	 * - 자바빈즈 프로퍼티를 조회하는경우 "속성명.프로퍼티명"을 지정한다.
