...
public class DatabaseConnectController {
	 ...
	 * 
	 * 			<bean id="jdbcTemplate" clas="org.springframework.jdbc.core.JdbcTemplate">
	 * 				<property name="dataSource" ref="dataSource">
	 * 			</bean>
	 * 
	 *  	3. 게시판 구현
	 *  
	 *  		기본적은 JDBC를 이용한 DB 설계는 DAO란에서 sql 쿼리가 노출되어 개발하는 방식이다보니 우리가 중급프로젝트때 또는
	 *  		JSP에서 preparestatement방식의 데이터베이스 연동과 다를바 없는 형태로, 게시판 구현을 진행하지 않고
	 *  		내용만 간략히 보고 넘어가도록 한다.
	 *  
	 ...
	 *   	설정하고 @Table어노테이션으로 테이블명을 설정해주면  VO클래스는 데이터베이스의 생산되어 있는 테이블과 연결되기 위한 준비가 된다.
	 *   	ex)
	 *   		@Entity
	 *   		@Table(name="board")
	 *   		public class Board{...
	 *   	그리고 각 필드(멤버변수)에 @Column(name="board_no") 어노테이션으로 테이블 컬럼과 매핑 설정을 한다.
	 *   	설정된 Entity들을 이용하여 jpa객체를 통해서 함수를 호출하면(persist, find 등등) 호출된 함수를통해 Spring Data JPA가
	 ...
	 *   	그리고, 사용하는 데이터베이스 종류가 변경된다 하더라도 문제가 없다. (JPA를 SQL 직접 건들이지 않고 함수와 같은 기능들로만 쿼리를 요청하고
	 *   	데이터베이스와 통신하기 때문입니다.)
	 *   
	 *   	JPA는 JPA구현체인 hibernate를 이용하여사용합니다. 그러기 위해선 관련 의존관계를등록 후 사용해야합니다.
	 *   	SQL문을 이용하여 데이터베이스를 연동하여 데이터를가공하는게 아닌, Method를 통해 데이터베이스를 조작할 수 있다는 장점이 있어 객체모델을
	 ...
	 *   [용어]
	 *   *** ORM이란?
	 *   	객체에 데이터를 읽고 쓰는 방법을 관계형 데이터베이스에 데이터를 읽고 쓰는 기술입니다.
	 *   *** 기업 면접에서 ORM은 뭐 쓰셨어요? 라는 질문을 받는 경우가 있다.
	 *   	  - 저는 ibatis를사용했습니다, 저는 mybatis를 사용했습니다 와 같은 대답을 하시면 됩니다.
	 *   ---------------------------------------------------
	 *   
	 *   7. 마이바티스
	 *   - SQL과 자바 객체를 매핑하는 아이디어로 개발된 데이터베이스 접근용 프레임워크다.
	 *   
	 *    마이바티스 관련 내용은 12장에서 더 자세하게 알고 넘어가도록합시다!
	 *    
	 *    	1) 의존 관계 정의
	 ...
	 *    	- 예제로 mapper패키지안에 BoardMapper.xml을 구성하여 테이스를 진행합니다!
	 */
}
