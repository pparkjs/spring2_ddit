...
public class MemberController {
	...
	// 7) 복수 선택이 가능한 폼 셀렉트 박스요소값을 문자열 배열 타입 매개변수로 처리한다.
	...
	// 12) 폼 체크박스 요속값을 기본 데이터 타입인 문자열 타입 매개변수로  처리한다.
	...
	// 16) 폼 텍스트 필드 요속값을 중첩된 자바빈즈 매개변수로 처리한다.
	...
	// 17) 폼 텍스트 영역 요소값을 기본 데이터 타입은 문자열 타입 매개변수로 처리한다.
	...
	public String registerUser(MemberAllForm member, Model model) {
		// 전체 폼 페이지에서 넘겨받은 데이터를 모두를 콘솔에 출력 후,
		...
		if(member.getGender() == "male") {
			member.setGender("남자");
		}else if(member.getGender() == "femail") {
			member.setGender("여자");
		}else {
			member.setGender("성소수자");
		}
		log.info("member.getGender() : " + member.getGender());
		if(member.getDeveloper() == "Y") {
			member.setDeveloper("개발자");	
		}else {
			member.setDeveloper("비전공자");	
		}
		log.info("member.getDeveloper() : " + member.getDeveloper());
		log.info("member.isForeigner() : " + member.isForeigner());
		if(member.getNationality().length > 0) {
			for(int i = 0; i < member.getNationality().length; i++) {
				String[] arr = new String[ member.getNationality().length];
				
				if(member.getNationality()[i] == "korea") {
					arr[i] = "대한민국"; 
				}else if(member.getNationality()[i] == "germany") {
					arr[i] = "독일";
				}else if(member.getNationality()[i] == "austrailia") {
					arr[i] = "호주";
				}else if(member.getNationality()[i] == "canada") {
					arr[i] = "캐나다";					
				}else {
					arr[i] = "미국";
				}
				
				member.setNationality(arr);
				log.info("Nationality : " + member.getNationality()[i]);
			}
		}else {
			log.info("Nationality is null");
		}
		if(member.getCars().length > 0) {
		...
		if(member.getHobby().length > 0) {
			String[] arr = new String[member.getHobby().length];
			for(int i = 0; i < member.getHobby().length; i++) {
				if(member.getHobby()[i].equals("sports")) {
					arr[i] = "운동";
				}else if(member.getHobby()[i].equals("music")){
					arr[i] = "음악";
				}else{
					arr[i] = "영화보기";
				}
				log.info("hobby["+i+"] :" + member.getHobby()[i]);
			}
			member.setHobby(arr);
		}else {
		...
		if(cardList.size() > 0 && cardList != null) {
			for(int i = 0; i< cardList.size(); i++) {
				log.info("cardList.getNo() : " + cardList.get(i).getNo());
				log.info("cardList.getValidMonth() : " + cardList.get(i).getValidMonth());
			}
	...
	// 3) 파일업로드 폼 파일 요소값과 텍스트 필드 요소값을  MultipartFile 매개변수의 자바빈즈 매개변수로 처리한다.
